...
def get_predicated_value(slope, intercept, independent_var):
    return float(intercept) + (float(slope) * float(independent_var))


def get_avg_error_rate(slope, intercept, data_points):
    error_rate = 0
    for i in data_points:
        actual_value = float(i[1])
        error_rate += (actual_value - get_predicated_value(slope, intercept, i[0])) ** 2
    return float(error_rate) / float(len(data_points))
